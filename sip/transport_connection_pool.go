...
func (p *connectionPool) Clear() error {
	p.Lock()
	conns := make([]Connection, 0, len(p.m))
	for _, c := range p.m {
	...
	var werr error
	for _, c := range conns {
		werr = errors.Join(werr, closePooledConnection(c))
	}
	return werr
}

func closePooledConnection(c Connection) (err error) {
	defer func() {
		if r := recover(); r != nil {
			err = errors.Join(err, fmt.Errorf("connection pool close panic: %v", r))
		}
	}()
	if c == nil {
		return nil
	}
	if _, closeErr := c.TryClose(); closeErr != nil {
