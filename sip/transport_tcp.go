...
func (t *TransportTCP) CreateConnection(ctx context.Context, laddr Addr, raddr Addr, handler MessageHandler) (Connection, error) {
	...
	conn, err := t.pool.addSingleflight(raddr, laddr, t.connectionReuse, func() (Connection, error) {
		...
		if t.DialContext != nil {
			...
			if err != nil {
				return nil, fmt.Errorf("%s custom dial err=%w", t, err)
			}
		} else {
...
func (c *TCPConnection) connAlive() bool {
	if c == nil {
		return false
	}
	conn := c.Conn
...
func (c *TCPConnection) Read(b []byte) (n int, err error) {
	...
	if SIPDebug {
		...
		logSIPRead("TCP", c.Conn.LocalAddr().String(), c.Conn.RemoteAddr().String(), out)
	}
	return n, err
}
...
func (c *TCPConnection) Write(b []byte) (n int, err error) {
	...
	if SIPDebug {
		...
		logSIPWrite("TCP", c.Conn.LocalAddr().String(), c.Conn.RemoteAddr().String(), out)
	}
	return n, err
}
