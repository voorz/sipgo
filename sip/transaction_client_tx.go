...
func (tx *ClientTx) Init() error {
	...
	// Timer B (INVITE) / Timer F backstop (non-INVITE). For reliable
	// non-INVITE requests the app layer enforces a shorter deadline; keep
	// sipgo's timer aligned so we never sit on the default 32s Timer_B.
	tx.mu.Lock()
	timeout := Timer_B
...
func (tx *ClientTx) ack() {
	...
	// https://github.com/voorz/sipgo/issues/168
